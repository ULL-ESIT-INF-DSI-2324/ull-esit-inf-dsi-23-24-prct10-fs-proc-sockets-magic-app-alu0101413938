...
export const mainServer = net.createServer((connection) => {
  ...
  connection.on('create', (cardInfo, callback) => {
    ...
    if (line == "planeswalker" || line == "creature" && special == "") {
      ...
      FileManager.Instance().writeOnFile(user, newCard, (refuse :boolean) => {
  ...
  connection.on('remove', (info, callback) => {
    const {user, id} = info;
    FileManager.Instance().removeFromFile(user, id, (refuse :boolean) => {
  ...
  connection.on('update', (cardInfo, callback) => {
    ...
    if (line == "planeswalker" || line == "creature" && special == "") {
      ...
      FileManager.Instance().updateFile(user, newCard, (refuse :boolean) => {
  ...
  connection.on('show', (info, callback) => {
    const {user, id} = info;
    FileManager.Instance().readFile(user, id, (refuse :boolean, showCard :CardData | undefined) => {
  ...
  connection.on('list', (info, callback) => {
    const {user} = info;
    FileManager.Instance().readMultipleFiles(user, (refuse :boolean, showCards :CardData[] | undefined) => {
